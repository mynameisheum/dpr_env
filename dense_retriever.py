        ...
        for j, batch_start in enumerate(range(0, n, bsz)):
            ...
            if query_token:
                ...
                if query_token == "[START_ENT]":
                    ...
                    # batch_tensors = [tensorizer.query_to_tensor(" ".join([query_token, q]), max_length=64) for q in batch_questions]
                ...
                # batch_tensors = [tensorizer.query_to_tensor(q, max_length = 64) for q in batch_questions]
...
class LocalFaissRetriever(DenseRetriever):
    ...
    def get_top_docs(self, query_vectors: np.array, top_docs: int = 100) -> List[Tuple[List[object], List[float]]]:
        ...
        :return:
        """
        print('start get top docs')
        time0 = time.time()
        # print(query_vectors,query_vectors.shape,query_vectors.dtype)
        results = self.index.search_knn(query_vectors, top_docs) # 10570
...
class DenseRPCRetriever(DenseRetriever):
    ...
    def get_top_docs(
        ...
        results = []
        for i in tqdm(range(0, query_vectors.shape[0], search_batch)):
            time0 = time.time()
...
def validate(
    ...
    match_stats = calculate_matches(passages, answers, result_ctx_ids, workers_num, match_type)   # 2626816
    top_k_hits = match_stats.top_k_hits  #

    logger.info("Validation results: top k documents hits %s", top_k_hits)
    top_k_hits = [v / len(result_ctx_ids) for v in top_k_hits] # top-1~100/ 10570
    logger.info("Validation results: top k documents hits accuracy %s", top_k_hits)
    print(f"Top-20: ",top_k_hits[19]*100) # 0.4635
    print(f"Top-100: ", top_k_hits[-1]*100) # 0.546
...
def save_results(
    ...
    #     shard_id = j
    #
    #     shard_size = math.ceil(len(questions)/ num_shard)
    ...
    #     print(start_idx)
    #     print(end_idx)
    #     for i, q in enumerate(questions[start_idx:end_idx]):
    #         # print(start_idx+i )
    #         q_answers = answers[start_idx + i]
    #         results_and_scores = top_passages_and_scores[start_idx + i]
    #         hits = per_question_hits[start_idx + i]
    ...
    print('appending end')
    with open(out_file[:out_file.rindex(".")] +"_"+ str(shard_id) + ".json", "w") as writer:
        writer.write(json.dumps(merged_data, indent=4) + "\n")
    logger.info("Saved results * scores  to %s", out_file[:out_file.rindex(".")] +"_"+ str(shard_id))
...
def iterate_encoded_files(vector_files: list, path_id_prefixes: List = None) -> Iterator[Tuple]:
    for i, file in enumerate(vector_files):
        ...
        with open(file, "rb") as reader:
            doc_vectors = pickle.load(reader) # 525384
            for doc in doc_vectors: # (525384 ,(wiki_i,768))
                doc = list(doc)
...
def get_all_passages2(ctx_sources):
    all_passages = {}
    # for ctx_src in ctx_sources:
    ctx_sources.load_data_to(all_passages)
...
def main(cfg: DictConfig):
    ...
    ## fixed ##
    # if ds_key == '/home/tjdgma/DPR/dpr/data/download_data/data/retriever/NQ/train_csv10.csv':
    #     import csv
    #     import ast
    #     f = open('/home/tjdgma/DPR/dpr/data/download_data/data/retriever/NQ/train_csv10.csv','r',)
    #     read_f = csv.reader(f, delimiter='\t')
    #     for i in read_f:
    #         print(i)
    #         questions.append(i[0])
    #         str_to_list = ast.literal_eval(i[1])
    #         question_answers.append(str_to_list)
    #     f.close()
    #     # df = pd.read_pickle('/home/tjdgma/DPR/dpr/data/download_data/data/retriever/NQ/train_pickle.pkl')
    ...
    if cfg.rpc_retriever_cfg_file and cfg.rpc_index_id:
        ...
        id_prefixes = []
        ctx_sources = []
        for ctx_src in cfg.ctx_datatsets: # ['dpr_wiki','dpr_wiki2','dpr_wiki3','dpr_wiki4'...]
            ctx_src = hydra.utils.instantiate(cfg.ctx_sources[ctx_src]) #{'dpr_wiki': {'_target_': 'dpr.data.retriever_data.CsvCtxSrc', 'file': 'data.wikipedia_split.psgs_w100', 'id_prefix': 'wiki:'}}
        ...
        for i, pattern in enumerate(ctx_files_patterns):
            pattern_files = glob.glob(pattern) # '/home/tjdgma/DPR/dpr/checkpoint_sqaud_1/inbatch/39_0_0'
            pattern_id_prefix = id_prefixes[i] # wiki
            input_paths.extend(pattern_files)
            path_id_prefixes.extend([pattern_id_prefix] * len(pattern_files)) # ['dpr_wiki']* n
    ...
    # get top k results
    print('get top k results iterative')
    # fix
    import math
    # # questions_tensors = []
    ...
    for j in range(100):
        ...
        top_results_and_scores = retriever.get_top_docs(questions_tensor[start_idx:end_idx].numpy(), cfg.n_docs)

        if cfg.use_rpc_meta: # false
            ...
            # fix #
            # all_passages = get_all_passages(ctx_sources) ## 위로 옮기기 ##

            # if  cfg.ctx_datatsets[0] == cfg.ctx_datatsets[1] :
            #     print('same ctx sources')
            #     all_passages = get_all_passages2(ctx_sources[0]) # [dpr_wiki, ...]
            # else :
            #     print('diff ctx source')
            #     all_passages = get_all_passages(ctx_sources)

            if cfg.validate_as_tables: # false
                ...
                questions_doc_hits = validate(
                    all_passages, #21025324
                    question_answers[start_idx:end_idx], #3610
                    top_results_and_scores, # (3610, 100)
                    cfg.validation_workers, # 16
                    cfg.match, # string
                )
            ...
            if cfg.out_file:
                save_results(
                    all_passages,
                    questions_text if questions_text else questions[start_idx:end_idx],
                    question_answers[start_idx:end_idx],
                    top_results_and_scores,
                    questions_doc_hits,
                    cfg.out_file,
                    shard_id=shard_id,
