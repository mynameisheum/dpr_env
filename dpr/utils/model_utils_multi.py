...
def get_schedule_linear(
    ...
    def lr_lambda(current_step):
        ...
        # steps_shift ==/ 835*1 = 10start 15 end/ 835*2 = 15start 20end/ 835*3 = 20start 25end/ 835*4 = 25start 30end/ 835*5 = 30start 35end /835*6 =35start 40end /835*7 =40start 45end/ 835*8 =45start 50end
        current_step += steps_shift
        # current_step += steps_shift
        if current_step < warmup_steps:
            # print(optimizer.param_groups[0]["lr"])
            # print('current_step ', current_step, ' < warmup steps', warmup_steps, ' : ',
            #         float(current_step) / float(max(1, warmup_steps)))
            return float(current_step) / float(max(1, warmup_steps))

        # print(optimizer.param_groups[0]["lr"])
        # print('current_step ', current_step, ' > warmup steps ', warmup_steps, ' : ', max(1e-7, float(
        #     total_training_steps - current_step) / float(max(1, total_training_steps - warmup_steps)), ))
...
def load_states_from_checkpoint(model_file: str) -> CheckpointState:
    ...
    logger.info("model_state_dict keys %s", state_dict.keys()) # okay
    # print(state_dict)
