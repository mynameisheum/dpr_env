...
def gen_ctx_vectors(
    ...
    n = len(ctx_rows) # 525384
    bsz = cfg.batch_size
    total = 0
    results = []
    for j, batch_start in enumerate(range(0, n, bsz)): # range(0, 525384, 32)
...
def main(cfg: DictConfig):
    ...
    all_passages_dict = {} # dpr_wiki
    ctx_src.load_data_to(all_passages_dict) # dpr_wiki
    all_passages = [(k, v) for k, v in all_passages_dict.items()]

    shard_size = math.ceil(len(all_passages) / cfg.num_shards) # total // 40 = 525384
    start_idx = cfg.shard_id * shard_size # 0 * 525384 = 0
    end_idx = start_idx + shard_size # 0 + 525384 = 525384
    ...
    shard_passages = all_passages[start_idx:end_idx] # [0:525384]

    data = gen_ctx_vectors(cfg, shard_passages, encoder, tensorizer, True)

    file = cfg.out_file + "_" + str(cfg.shard_id) # 0
