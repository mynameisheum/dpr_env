...
#         save_cp = cfg.local_rank in [-1, 0] # True
#
...
class BiEncoderTrainer(object):
    ...
    def run_train(self):
        ...
        for epoch in range(self.start_epoch, int(cfg.train.num_train_epochs)):
            logger.info("***** Epoch %d *****", epoch)
            logger.info("Training finished. Best validation checkpoint %s", self.best_cp_name)
            print(register_epoch_loss)
            # print(neg_in_neg_count)
            self._train_epoch(scheduler, epoch, eval_step, train_iterator)
    ...
    def validate_and_save(self, epoch: int, iteration: int, scheduler):
        ...
        # if epoch in [0,1,25,30,35,39]:
        cfg = self.cfg
        # for distributed mode, save checkpoint for only one process
        save_cp = cfg.local_rank in [-1, 0] # -1

        if epoch == cfg.val_av_rank_start_epoch: # 실제 추론을 통해 rank 성능으로 모델저장하기
            self.best_validation_result = None

        if not cfg.dev_datasets: # dev set 없으면 그냥 0으로 유지
            validation_loss = 0 # 1472.0357
        else: # dev set 잇으면 여기
            if epoch >= cfg.val_av_rank_start_epoch: # 실제 추론을 통해 rank 성능으로 모델저장하기
                validation_loss = self.validate_average_rank()
            else:                                    # 그전까지 negative log likelihood를 통해 loss 값으로만 판단해서 모델 저장하기
                validation_loss = self.validate_nll()
    ...
    def validate_nll(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            biencoder_input = biencoder.create_val_biencoder_input(
                samples_batch,
                self.tensorizer,
                True,
                num_hard_negatives,
    ...
    def validate_average_rank(self) -> float:
        ...
        for i, samples_batch in enumerate(data_iterator.iterate_ds_data()):
            ...
            total_ctxs = len(ctx_represenations)
            ctxs_ids = biencoder_input.context_ids.cuda()
            ctxs_segments = biencoder_input.ctx_segments.cuda()
            bsz = ctxs_ids.size(0)

            # get the token to be used for representation selection
            ds_cfg = self.ds_cfg.dev_datasets[dataset]
            encoder_type = ds_cfg.encoder_type
            rep_positions = ds_cfg.selector.get_positions(biencoder_input.question_ids.cuda(), self.tensorizer)

            # split contexts batch into sub batches since it is supposed to be too large to be processed in one batch
            for j, batch_start in enumerate(range(0, bsz, sub_batch_size)):

                q_ids, q_segments = (
                    (biencoder_input.question_ids.cuda(), biencoder_input.question_segments.cuda()) if j == 0 else (None, None)
                )
                ...
                ctx_ids_batch = ctxs_ids[batch_start : batch_start + sub_batch_size].cuda()
                ctx_seg_batch = ctxs_segments[batch_start : batch_start + sub_batch_size].cuda()
        ...
        ctx_represenations = torch.cat(ctx_represenations, dim=0) # (482852,738)
        q_represenations = torch.cat(q_represenations, dim=0) # (7920,768), shape = (1,768)
        ...
        scores = sim_score_f(q_represenations, ctx_represenations) # 7920,482852
        values, indices = torch.sort(scores, dim=1, descending=True)
        # values = (7920, 482852), indices = (7920,482852)
        rank = 0
        for i, idx in enumerate(positive_idx_per_question): # 482791
            # aggregate the rank of the known gold passage in the sorted results for each question
            gold_idx = (indices[i] == idx).nonzero()
            rank += gold_idx.item()
    ...
    def _train_epoch(
        ...
        for i, samples_batch in enumerate(train_data_iterator.iterate_ds_data(epoch=epoch)):
            ...
            biencoder_batch, num_idx , neg_in_neg = biencoder.create_biencoder_input(
            # biencoder_batch = biencoder.create_val_biencoder_input(
            #     samples_batch,
            #     self.tensorizer,
            #     True,
            #     num_hard_negatives,
            ...
            samples_batch,
            self.tensorizer,
            True, # multidoc2dial kb만들때도 제목 중복인채로 embedding 만듬
            num_hard_negatives,
            num_other_negatives,
            # shuffle=False,
            shuffle_positives=shuffle_positives,
            ...
            if cfg.fp16:
                ...
                loss.backward()
                if cfg.train.max_grad_norm > 0: # 2.0이고
                    torch.nn.utils.clip_grad_norm_(self.biencoder.parameters(), cfg.train.max_grad_norm)
            ...
            if i % log_result_step == 0:
                ...
                # print(lr)
                logger.info(
                    "Epoch: %d: Step: %d/%d, loss=%f, lr=%f or lr=%.13f",
                    epoch,
                    data_iteration,
                    epoch_batches,
                    loss.item(),
            ...
            # if epoch >= cfg.train.num_train_epochs-5:
            # if epoch in [0,39]:
            if data_iteration % eval_step == 0:
                logger.info(
                    "rank=%d, Validation: Epoch: %d Step: %d/%d",
        ...
        # if epoch == cfg.train.num_train_epochs-5:
        # if epoch in [0, 25,30,35,39,49]:
        logger.info("Epoch finished on %d", cfg.local_rank)
        self.validate_and_save(epoch, data_iteration, scheduler)

        epoch_loss = (epoch_loss / epoch_batches) if epoch_batches > 0 else 0
        register_epoch_loss.append(epoch_loss)
        # neg_in_neg_count.append(neg_in_neg)
    ...
    def _save_checkpoint(self, scheduler, epoch: int, offset: int) -> str:
        ...
        #     exit()
        # if epoch in [0,39]:
        # if epoch in [0,15,30,47]:
        # if epoch in [10,20,30,40,48,49,]:
        # if epoch in [2,5,10,20,30,49]:
    ...
    def _load_saved_state(self, saved_state: CheckpointState):
        ...
        if self.cfg.ignore_checkpoint_offset: # False or True
            self.start_epoch = 0
            self.start_batch = 0
        else:
            self.start_epoch = epoch
        ...
        model_to_load.load_state(saved_state, strict=True)
        logger.info("Saved state loaded")
        if not self.cfg.ignore_checkpoint_optimizer: # false
            if saved_state.optimizer_dict:
                logger.info("Using saved optimizer state")
                self.optimizer.load_state_dict(saved_state.optimizer_dict)

        if not self.cfg.ignore_checkpoint_lr and saved_state.scheduler_dict: # true false
            logger.info("Using saved scheduler_state")
            self.scheduler_state = saved_state.scheduler_dict
...
def _calc_loss(
    ...
    else: # distributed_world_size = 1일때
        global_q_vector = local_q_vector # batch size : 768 CLS
        global_ctxs_vector = local_ctx_vectors # batch size : 768 CLS
        positive_idx_per_question = local_positive_idxs # positive label ( 0,4 ,8 ,12)
        hard_negatives_per_question = local_hard_negatives_idxs # negative label(other neg가 아니라 hard neg라 0으로 들어감)

    loss, is_correct = loss_function.calc(
        global_q_vector, # batch size : 768 CLS
        global_ctxs_vector, # batch size : 768 CLS
        positive_idx_per_question, # positive label
        hard_negatives_per_question, # negative label
        loss_scale=loss_scale,
    )

    return loss, is_correct # loss와 back propagation 되기 이전에 batch size중에서 맞춘 개수
...
def _do_biencoder_fwd_pass(
    ...
    input = BiEncoderBatch(**move_to_device(input._asdict(), cfg.device)) # 여기서 Memory 걸림!!!!!!

    q_attn_mask = tensorizer.get_attn_mask(input.question_ids) # batch size : 128 token
    ctx_attn_mask = tensorizer.get_attn_mask(input.context_ids) # batch size : 512 token

    if model.training:
        model_out = model(
            input.question_ids, # batch size : 128 token
            input.question_segments, # batch size : 128 zero
            q_attn_mask, # batch size : 128 attn mask
            input.context_ids, # batcn size : 512 token
            input.ctx_segments, # batch size : 512 zero
            ctx_attn_mask, # batch size : 512 attn mask
            encoder_type=encoder_type, # none
            representation_token_pos=rep_positions, # 0
        )
    # if RS_sampling :
    #     model_out2 = model(
    #         input.question_ids,  # batch size : 128 token
    ...
    local_q_vector, local_ctx_vectors = model_out
    # (batch q : CLS) , (batch * (q+hard) : CLS)
    loss_function = BiEncoderNllLoss()

    loss, is_correct = _calc_loss(
        cfg,
        loss_function,
        local_q_vector, # batch size : 768 CLS
        local_ctx_vectors, # batch size : 768 CLS
        input.is_positive, # positive label
        input.hard_negatives, # neg label
        loss_scale=loss_scale,
    )
    is_correct = is_correct.sum().item()
